...
var getRawType = require('graphql-compiler').SchemaUtils.getRawType,
    isAbstractType = require('graphql-compiler').SchemaUtils.isAbstractType,
    getNullableType = require('graphql-compiler').SchemaUtils.getNullableType;
...
function generate(node) {
  !(['Batch', 'Fragment'].indexOf(node.kind) >= 0) ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayCodeGenerator: Unknown AST kind `%s`. Source: %s.', node.kind, getErrorMessage(node)) : require('fbjs/lib/invariant')(false) : void 0;
  return require('graphql-compiler').IRVisitor.visit(node, RelayCodeGenVisitor);
}

var RelayCodeGenVisitor = {
  leave: {
    Batch: function Batch(node) {
      !(node.requests.length !== 0) ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'Batch must contain Requests.') : require('fbjs/lib/invariant')(false) : void 0;
    ...
    Condition: function Condition(node, key, parent, ancestors) {
      !(node.condition.kind === 'Variable') ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayCodeGenerator: Expected static `Condition` node to be ' + 'pruned or inlined. Source: %s.', getErrorMessage(ancestors[0])) : require('fbjs/lib/invariant')(false) : void 0;
      return {
        kind: 'Condition',
...
function isPlural(type) {
  return getNullableType(type) instanceof require('graphql').GraphQLList;
}

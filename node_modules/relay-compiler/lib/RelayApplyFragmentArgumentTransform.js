...
function relayApplyFragmentArgumentTransform(context) {
  ...
  var nextContext = require('graphql-compiler').IRTransformer.transform(context, {
...
function transformCondition(context, fragments, scope, node) {
  ...
  !(condition.kind === 'Literal' || condition.kind === 'Variable') ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayApplyFragmentArgumentTransform: A non-scalar value was applied to ' + 'an @include or @skip directive, the `if` argument value must be a ' + 'variable or a Boolean, got `%s`.', condition) : require('fbjs/lib/invariant')(false) : void 0;
...
function transformValue(scope, value) {
  if (value.kind === 'Variable') {
    ...
    !(scopeValue != null) ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayApplyFragmentArgumentTransform: variable `%s` is not in scope.', value.variableName) : require('fbjs/lib/invariant')(false) : void 0;
    return scopeValue;
...
function transformFragment(context, fragments, parentScope, fragment, args) {
  ...
  var fragmentScope = require('./RelayCompilerScope').getFragmentScope(fragment.argumentDefinitions, args, parentScope, fragment.name);
  !(!fragments.has(fragmentName) || fragments.get(fragmentName) != null) ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayApplyFragmentArgumentTransform: Found a circular reference from ' + 'fragment `%s`.', fragment.name) : require('fbjs/lib/invariant')(false) : void 0;
...
function hashArguments(args, scope) {
  ...
  var printedArgs = JSON.stringify(sortedArgs.map(function (arg) {
    ...
    if (arg.value.kind === 'Variable') {
      ...
      !(value != null) ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayApplyFragmentArgumentTransform: variable `%s` is not in scope.', arg.value.variableName) : require('fbjs/lib/invariant')(false) : void 0;
    } else {
      value = arg.value;
    }
    return {
      name: arg.name,
      value: require('graphql-compiler').getIdentifierForArgumentValue(value)
    };
  }));

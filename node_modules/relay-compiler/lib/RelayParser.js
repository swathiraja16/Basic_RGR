...
var RelayParser = function (_Parser) {
  ...
  return RelayParser;
}(require('graphql-compiler').Parser);

function getName(ast) {
  var name = ast.name ? ast.name.value : null;
  !(typeof name === 'string') ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayParser: Expected ast node `%s` to have a name.', ast) : require('fbjs/lib/invariant')(false) : void 0;
  return name;
}

...
var canHaveSelections = require('graphql-compiler').SchemaUtils.canHaveSelections,
    getRawType = require('graphql-compiler').SchemaUtils.getRawType,
    hasID = require('graphql-compiler').SchemaUtils.hasID,
    implementsInterface = require('graphql-compiler').SchemaUtils.implementsInterface,
    isAbstractType = require('graphql-compiler').SchemaUtils.isAbstractType,
    mayImplement = require('graphql-compiler').SchemaUtils.mayImplement;
...
function relayGenerateIDFieldTransform(context) {
  var idType = require('graphql').assertLeafType(context.serverSchema.getType(ID_TYPE));
  var idField = {
    kind: 'ScalarField',
    alias: null,
    args: [],
...
function visitLinkedField(field, state) {
  ...
  if (require('./RelayTransformUtils').hasUnaliasedSelection(field, ID)) {
    return transformedNode;
  }

  var context = this.getContext();
  var schema = context.serverSchema;
  var unmodifiedType = require('graphql').assertCompositeType(getRawType(field.type));
  ...
  if (isAbstractType(unmodifiedType)) {
    ...
    if (mayImplement(schema, unmodifiedType, NODE_TYPE)) {
      var nodeType = require('graphql').assertCompositeType(schema.getType(NODE_TYPE));
      selections.push(buildIDFragment(nodeType, state.idField));
    }
    var abstractType = require('graphql').assertAbstractType(unmodifiedType);

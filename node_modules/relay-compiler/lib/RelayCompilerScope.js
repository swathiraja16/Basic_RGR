...
function getFragmentScope(definitions, args, parentScope, fragmentName) {
  ...
  definitions.forEach(function (definition) {
    if (definition.kind === 'RootArgumentDefinition') {
      !!argMap.hasOwnProperty(definition.name) ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayCompilerScope: Unexpected argument for global variable `%s` ' + 'for `%s`. @arguments may only be provided for variables ' + "defined in the fragment's @argumentDefinitions list.", definition.name, fragmentName) : require('fbjs/lib/invariant')(false) : void 0;
      ...
      if (arg == null || arg.kind === 'Literal' && arg.value == null) {
        ...
        // value.
        !(definition.defaultValue != null || !(definition.type instanceof require('graphql').GraphQLNonNull)) ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayCompilerScope: No value found for required argument ' + '`$%s: %s` in `%s`.', definition.name, definition.type.toString(), fragmentName) : require('fbjs/lib/invariant')(false) : void 0;
        fragmentScope[definition.name] = {
          kind: 'Literal',
